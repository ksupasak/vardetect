 ...
 * @author worawich
 */
public class RunVariantDetectionFullProcessV2 {
    /**
     ...
     * Add create reference process 
     * not finish wait for full protocol
     */
...
//        boolean annotationFlag = Boolean.valueOf(args[9]);              // Anotation flag true = do annotate ; false = not do
        ...
        String refFaIndex = args[0]+".fai";
        String refFile = args[0];
        String sampleFaIndex = args[1]+".fai";
        String sampleFile = args[1];
        int maxInterestCoverage = Integer.valueOf(args[12]);
        int refExtendSize = Integer.valueOf(args[13]);
...
//        varRes.writeVariantCoverageVirtualizeWithAnnotationReportToFile(filename2, gffFile, coverageThreshold, 'I');
        ...
        // create Reference from variation events
        
        //indelType (SI = small Insertion, SD = small deletion, LI = large Indel)
        varRes.createReferenceFromNovelIndelResult(filename, refFile, refFaIndex, sampleFile, sampleFaIndex, "SI", maxInterestCoverage, refExtendSize);
        varRes.createReferenceFromNovelIndelResult(filename, refFile, refFaIndex, sampleFile, sampleFaIndex, "SD", maxInterestCoverage, refExtendSize);
        varRes.createReferenceFromNovelIndelResult(filename, refFile, refFaIndex, sampleFile, sampleFaIndex, "LI", maxInterestCoverage, refExtendSize);
